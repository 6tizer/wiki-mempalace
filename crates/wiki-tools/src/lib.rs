...
pub struct ToolContext<'a> {
    pub eng: &'a mut LlmWikiEngine<NoopWikiHook>,
    pub repo: &'a SqliteRepository,
    pub viewer: &'a Scope,
    pub llm_config_path: &'a std::path::Path,
    pub vectors: bool,
    pub wiki_dir: Option<&'a std::path::Path>,
...
impl ToolRegistry {
    ...
    pub fn call(
        ...
        self.call_params(json!({"name": name, "arguments": args}), ctx)
    }

    pub fn call_params(&self, params: Value, ctx: ToolContext<'_>) -> Result<Value, McpToolError> {
        call_tool(
            params,
            ctx.eng,
            ctx.repo,
...
pub fn parse_tier(s: &str) -> Result<MemoryTier, Box<dyn std::error::Error>> {
...
fn handle_request(
    ...
    let result = match method {
        ...
        "tools/list" => Ok(registry.list_json()),
        "tools/call" => registry.call_params(
            params,
            ToolContext {
                eng,
                repo,
...
fn call_tool(
    ...
    match name {
        ...
        "wiki_ingest_llm" => {
            ...
            let cfg = wiki_ai::llm::load_llm_config(llm_config_path).map_err(McpToolError::llm)?;
            let user_msg = wiki_ai::llm::build_ingest_llm_user_prompt(&cfg, uri, body)
                .map_err(McpToolError::llm)?;
            let reply = wiki_ai::llm::complete_chat_json_object(
                &cfg,
                wiki_ai::llm::ingest_llm_system_prompt(),
                &user_msg,
                cfg.max_output_tokens.min(8192),
            )
            .map_err(McpToolError::llm)?;
            let slice = wiki_ai::llm::parse_json_object_slice(&reply);
            let plan: wiki_core::LlmIngestPlanV1 = serde_json::from_str(slice).map_err(|e| {
                McpToolError::llm(format!(
                    "JSON parse error: {e}; raw={}",
                    wiki_ai::llm::redact_for_llm_error(&reply)
                ))
            })?;
            ...
            let sid = eng
                ...
                .map_err(McpToolError::engine)?;
            let app = if vectors {
                Some(wiki_ai::llm::load_app_config(llm_config_path).map_err(McpToolError::llm)?)
            } else {
                None
            };
            ...
            if let Some(app) = &app {
                ...
                let v = wiki_ai::llm::embed_first(app, &short).map_err(McpToolError::llm)?;
            ...
            for c in &plan.claims {
                ...
                eng.attach_sources(cid, &[sid])
                    .map_err(McpToolError::engine)?;
                if let Some(app) = &app {
                    let v = wiki_ai::llm::embed_first(app, &c.text).map_err(McpToolError::llm)?;
...
fn build_source_embedding(
    ...
    let app = wiki_ai::llm::load_app_config(llm_config_path)?;
    let short: String = body.chars().take(16000).collect();
    let vec = wiki_ai::llm::embed_first(&app, &short)?;
...
fn build_text_embedding(
    ...
    let app = wiki_ai::llm::load_app_config(llm_config_path)?;
    let vec = wiki_ai::llm::embed_first(&app, text)?;
...
mod tests {
    ...
    #[test]
    fn tool_registry_lists_twenty_two_tools_once() {
        let registry = ToolRegistry;
        ...
        assert!(names.contains(&"wiki_query"));
