...
| M11 运维控制台 | 🟡 本分支已实现 / integration gate passed | 已新增 `wiki-cli dashboard`，默认输出 `wiki/reports/dashboard.html`，支持 `--output <PATH>`、`--consumer-tag <TAG>`、`--low-coverage-threshold <N>`；生成静态自包含 HTML；workspace fmt/test/clippy 已通过 | 等待 draft PR / CI |
...
- 当前状态：focused tests passing，integration review / workspace gate 已通过，draft PR / CI 仍 pending。
