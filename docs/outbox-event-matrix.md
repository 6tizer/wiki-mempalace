...
| `QueryServed` | retained-no-consumer | `wiki-kernel::LlmWikiEngine::record_query` / `query_pipeline_memory` | CLI `query` | `query_fingerprint`（兼容字段，新事件填 hash）, `query_hash`, `query_hash_schema_version`, `schema_version`, `viewer_scope`, `redacted_preview`, `top_doc_ids`, `at` | `wiki-cli suggest` / M12 strategy scan 只读消费 query history | no | bridge 统计为 `ignored` | 新事件不保存原始 query；旧事件缺 hash/scope 时仍可读取，M12 只在 scope/doc 可见性可证明时使用 | `crates/wiki-core/src/events.rs` `query_served_serializes_hash_without_raw_query`、`legacy_query_served_deserializes_without_new_fields`；`crates/wiki-kernel/src/engine.rs` `record_query_emits_query_served_event`；`crates/wiki-kernel/src/strategy.rs` `scoped_query_served_requires_event_scope_to_match_viewer`；`crates/wiki-cli/tests/query_privacy.rs` `query_outbox_hashes_query_and_records_viewer_scope`；`crates/wiki-mempalace-bridge/src/lib.rs` `stats_mark_unresolved_and_ignored_events` |
