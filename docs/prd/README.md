...
- [compiler-canonicalization-v2.md](compiler-canonicalization-v2.md) — Pre-write compiler resolver: bounded candidate retrieval, persisted alias/canonical mapping, small LLM fallback, and DB-first post-write governance before production scale-up.
- [production-wiki-compiler.md](production-wiki-compiler.md) — Notion-equivalent local compiler and canonicalization v2 follow-up: raw source -> summary + concept/entity pages -> Vault -> outbox -> Mempalace -> query/explain.
