...
| 9 | CJK / Unicode retrieval | 🚧 Active branch | P2 | M-6 | `rust-mempalace` FTS query 保留 Unicode token；CJK 走 trigram/LIKE fallback；空 token 不再固定成 `"memory"`。 | 中文 query e2e 通过；英文 FTS quote 回归不退化；搜索 limit 仍受 PR 1 clamp 保护。 | PR 1 |
