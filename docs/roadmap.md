...
1. B5 orphan governance：draft PR #28 已开；`codex/orphan-governance` 已实现只读治理报告；基于 2026-04-25 生产 `vault-audit` 的 4 个 orphan candidates、12 个 unsupported frontmatter、5 个缺 `status` page、16 个缺 `compiled_to_wiki` source 做分类，不清理 vault。
