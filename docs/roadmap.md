...
| M10 指标与评估 | 🔎 In Review / 已实现待 CI | `wiki-cli metrics` 已实现；支持 `--consumer-tag`、`--low-coverage-threshold`、`--json`、`--report <PATH>`；覆盖 content/lint/gaps/outbox/lifecycle 5 组指标；core/kernel/cli metrics 测试初步通过 |
...
1. M10 指标与评估：已实现待 review/CI，后续确认 metrics 口径稳定。
