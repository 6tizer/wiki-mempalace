...
| 3 | Mempalace bank capability | ✅ Gate passed | P0 | H-2 / I-3 | MCP mempalace 工具禁止 client 传 `bank_id`；由 `viewer_scope -> bank_id` 派生；KG query/timeline/stats 加 bank filter。 | 搜索、wake_up、taxonomy、traverse、reflect、KG 查询均按 bank 隔离；越权测试通过。 | PR 2 |
