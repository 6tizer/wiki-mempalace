...
| Notion Incremental Sync | ✅ 已合入 | PR #36；`wiki-cli notion-sync`、automation `notion-sync` daily job 已实现；增量游标 `notion_sync_cursors`/`notion_page_index`；速率限制 350ms + 429 重试；`--writeback-notion` 接口完整默认关闭；PRD: `docs/prd/notion-incremental-sync.md`；smoke test: X书签 782 + 微信文章 482 dry-run 通过 |
...
2. **Notion Incremental Sync**：PR #36 已合并；首轮生产执行 `notion-sync --db-id all` 待安排。
