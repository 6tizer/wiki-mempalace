...
| M11 运维控制台 | 🟡 本分支已实现 / integration gate passed | `wiki-cli dashboard` 已实现；默认输出 `wiki/reports/dashboard.html`，支持 `--output <PATH>`、`--consumer-tag <TAG>`、`--low-coverage-threshold <N>`；生成静态自包含 HTML，无 web server / 外部 CSS/JS；不依赖 palace DB；默认只读，等待 draft PR / CI |
...
2. M11 运维控制台：本分支已实现最小只读 dashboard / 本地报告页，integration gate 已通过，等待 draft PR / CI。
