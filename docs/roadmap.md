...
| 6 | wiki-cli LLM governance | 🚧 Active branch | P1 | M-2 / I-4 | `wiki-cli` LLM config 支持 `api_key_env`；prompt 中 source body 明确作为 untrusted payload；补 max input/output、redaction hook、provider allowlist、structured validation。 | inline key 兼容但非首选；adversarial prompt / oversized input / invalid output 测试通过。 | PR 1 |
