...
| 轨道 | 状态 | 当前事实 |
| --- | --- | --- |
| Active implementation | PR2 执行中 | Audit Report Follow-up v2 已由 PR #83-#93 全部合入；2026-04-30 处置决策新增 3 个待修复 PR，PR #95 已合入，当前执行 PR2 docs consistency |
...
| 2 | Docs consistency cleanup | Active branch | P1 | L-4 | 统一 Rust edition 描述和 Notion 增量同步状态；修 `docs/architecture.md`、`crates/rust-mempalace/README.md` 等明显矛盾。 | active docs 不再命中旧 edition 说法；active docs 不再出现 Notion sync 旧状态；docs-only `git diff --check` 过。 | `codex/audit-disposition-02-doc-consistency` |
