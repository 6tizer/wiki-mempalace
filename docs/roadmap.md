...
| 4 | Cross-bank drawer dedupe migration | 🚧 Active branch | P0 | M-7 | `drawers(content_hash)` 全局唯一改为 `(bank_id, content_hash)`；live sink / mine path 查重同步改造。 | migration 可重复执行；相同内容不同 bank 可共存；同 bank 仍去重；bridge/live tests 通过。 | PR 3 |
