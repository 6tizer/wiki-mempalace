...
| 8 | Production SearchPorts default | 🚧 Active branch | P2 | M-5 | 生产 query 默认使用 storage-backed BM25/vector/graph ports；`InMemorySearchPorts` 限定测试/fallback。 | 无 palace/storage 时行为有明确 fallback；query truth table 文档同步；CLI/MCP query tests 通过。 | PR 5 |
