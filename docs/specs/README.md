...
- [production-wiki-compiler/](production-wiki-compiler/) — Notion-equivalent local compiler: source -> summary + concept/entity pages with backlinks. Implementation merged; tiny sample exposed the next follow-up, compiler canonicalization v2.
- [compiler-canonicalization-v2/](compiler-canonicalization-v2/) — Pre-write compiler canonical resolver with bounded DB candidates, persisted aliases, small LLM fallback, and temp-vault regression before production scale-up.
