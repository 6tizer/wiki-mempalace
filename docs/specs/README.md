...
- [persist-snapshot-outbox/](persist-snapshot-outbox/) — C16a: `wiki_state` + outbox append in one SQLite transaction; replaces split `save_to_repo` / `flush_outbox` autocommit for crash-safety. PRD: [storage-embeddings-followup.md](../prd/storage-embeddings-followup.md).
