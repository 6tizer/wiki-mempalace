...
- [x] Outbox Consumer Cursors — PR #62/#63 已完成 consumer-scoped export API 与 CLI/consumer cutover（roadmap: "Outbox Consumer Cursors"）
- [x] Embedding Tx Atomicity — `upsert_embedding` 纳入 snapshot+outbox 同一事务（PR #71）
- [x] Benchmark Reproducibility — `--seed` 参数存入 `benchmark_runs`（PR #70）
